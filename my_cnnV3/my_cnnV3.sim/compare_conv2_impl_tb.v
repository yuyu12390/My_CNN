...
module compare_conv2_impl_tb;
    ...
    reg signed [7:0] direct_din_0;
    reg signed [7:0] direct_din_1;
    ...
    initial begin
        ...
        for (i = 0; i < 385; i = i + 1) begin
            ...
            if ((i >= WINDOW_START_DELAY) && (i < WINDOW_START_DELAY + 144)) begin
                fmap_valid <= 1'b1;
                din_0 <= fmap0_mem[i - WINDOW_START_DELAY];
                din_1 <= fmap1_mem[i - WINDOW_START_DELAY];
                if ((i - WINDOW_START_DELAY + 3) < 144) begin
                    direct_din_0 <= fmap0_mem[i - WINDOW_START_DELAY + 3];
                    direct_din_1 <= fmap1_mem[i - WINDOW_START_DELAY + 3];
                end
                else begin
                    direct_din_0 <= 8'sd0;
                    direct_din_1 <= 8'sd0;
        ...
        if (stream_count != 64) begin
            $display("STREAM count mismatch: %0d", stream_count);
            err_count = err_count + 1;
        end
        if (direct_count != 64) begin
            $display("DIRECT count mismatch: %0d", direct_count);
            err_count = err_count + 1;
        end
        ...
        if (err_count == 0)
            ...
            $display("COMPARE FAIL, err_count=%0d", err_count);
