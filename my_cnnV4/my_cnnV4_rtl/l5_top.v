...
// 2. 只把第五层目标 (2,0) ~ (2,9) 翻译给 10 个 fc_neuron
// 3. 复用 fc_addr_mgr, 从上一级 12 路 4x4 缓存顺序读出 32 拍输入
// 4. 10 个神经元并行计算, 一次输出 10 个分类分数
module l5_top
#(
...
(
    ...
    input  [IN_CH_NUM-1:0] src_frame_valid,                    // 上一级 12 路输入帧有效
    input  signed [IN_CH_NUM*DATA_WIDTH-1:0] src_rd_data,      // 上一级 12 路读回数据
    input  [IN_CH_NUM-1:0] src_rd_valid,                       // 上一级 12 路读回有效
    ...
    output weight_loaded,                                      // 10 个神经元权重全部装好
    output [IN_CH_NUM-1:0] src_rd_en,                          // 读上一级 12 路缓存使能
    output [IN_CH_NUM*ADDR2D_WIDTH-1:0] src_rd_addr2d,         // 读上一级 12 路缓存地址
    output reg [IN_CH_NUM-1:0] src_rd_done,                    // 释放上一级 12 路输入帧
    output score_valid,                                        // 10 路结果整体有效
    output signed [OUT_NUM*OUT_WIDTH-1:0] score_data           // 10 路分类分数
);
    ...
    assign gw_is_l5_target = (gw_layer_id == LAYER2_ID) && (gw_kernel_id < L2_KERNEL_NUM);

    assign l5_cfg_weight_valid = gw_weight_valid && gw_is_l5_target;
    assign l5_cfg_weight_data = gw_weight_data;
    assign l5_cfg_weight_last = gw_dst_last && gw_is_l5_target;
    assign l5_cfg_weight_out = l5_cfg_weight_out_reg;

    assign gw_weight_ready = gw_is_l5_target ? neuron_cfg_weight_ready_vec[l5_cfg_weight_out]
                                             : 1'b1;
    ...
    begin
        ...
        if(gw_kernel_id < L2_KERNEL_NUM)
        begin
            l5_cfg_weight_out_reg = gw_kernel_id[OUT_SEL_WIDTH-1:0];
        end
    end
    ...
    generate
        ...
        begin: g_fc_neuron
            ...
            fc_neuron #(
                ...
                .TOTAL_WEIGHT_NUM(L2_WEIGHT_NUM),
                .OUT_WIDTH(OUT_WIDTH)
            ) u_fc_neuron (
                .clk(clk),
                .rstn(rstn),
