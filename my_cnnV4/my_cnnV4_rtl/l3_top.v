...
// 4. 上一级 6 路输入缓存和下一级 12 路输出缓存接口仍复用现有 l3_core
module l3_top
#(
    parameter SRC_NUM = 6,
...
(
    ...
    reg [SRC_SEL_WIDTH-1:0] l3_cfg_weight_cin_reg;
    reg [OUT_SEL_WIDTH-1:0] l3_cfg_weight_out_reg;
    reg l3_cfg_weight_last_sel_reg;

    integer l3_raw_weight_idx_int;
    integer l3_raw_block_idx_int;
    integer l3_raw_group_idx_int;
    integer l3_raw_lane_idx_int;
    integer l3_raw_tap_idx_int;
    ...
    // 第三层目标走本地 12 路输出核握手, 其他目标当前阶段直接吞掉
    assign gw_weight_ready = gw_is_l3_target ? l3_cfg_weight_ready : 1'b1;

    assign l3_cfg_weight_valid = gw_weight_valid && gw_is_l3_target;
    assign l3_cfg_weight_data = gw_weight_data;
    ...
    always @(*)
    begin
        l3_cfg_weight_cin_reg = {SRC_SEL_WIDTH{1'b0}};
        l3_cfg_weight_out_reg = {OUT_SEL_WIDTH{1'b0}};
        l3_cfg_weight_last_sel_reg = 1'b0;
        l3_raw_weight_idx_int = 0;
        l3_raw_block_idx_int = 0;
        l3_raw_group_idx_int = 0;
        l3_raw_lane_idx_int = 0;
        l3_raw_tap_idx_int = 0;

        if(gw_is_l3_target)
        begin
            // third-layer raw stream order: cin-major, then out-group, then lane, then 5x5 taps
            l3_raw_weight_idx_int = (gw_kernel_id * L1_WEIGHT_NUM) + gw_weight_idx;
            ...
            l3_cfg_weight_cin_reg = l3_raw_weight_idx_int / (L1_KERNEL_NUM * L0_WEIGHT_NUM);
            l3_cfg_weight_out_reg = (l3_raw_group_idx_int * L0_KERNEL_NUM) + l3_raw_lane_idx_int;
            l3_cfg_weight_last_sel_reg = (l3_raw_tap_idx_int == (L0_WEIGHT_NUM - 1));
        end
    end
