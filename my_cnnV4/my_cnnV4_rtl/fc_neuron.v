...
(
    ...
    integer lane_idx;
    integer group_idx_int;
    integer beat_pos_int;
    integer weight_sel_idx_int;
    ...
    begin
        ...
        group_idx_int = beat_cnt / BEATS_PER_GROUP;
        beat_pos_int = beat_cnt - (group_idx_int * BEATS_PER_GROUP);

        for(lane_idx = 0; lane_idx < LANE_NUM; lane_idx = lane_idx + 1)
        begin
            weight_sel_idx_int = (group_idx_int * LANE_NUM * BEATS_PER_GROUP)
                               + (lane_idx * BEATS_PER_GROUP)
                               + beat_pos_int;

            beat_sum_comb = beat_sum_comb
                          + $signed(
                                $signed({{WEIGHT_WIDTH{in_data[((lane_idx + 1) * DATA_WIDTH) - 1]}},
                                         in_data[((lane_idx + 1) * DATA_WIDTH) - 1 -: DATA_WIDTH]})
                              * $signed({{DATA_WIDTH{weight_mem[weight_sel_idx_int][WEIGHT_WIDTH - 1]}},
                                         weight_mem[weight_sel_idx_int]})
                            );
        end
